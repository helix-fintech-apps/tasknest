...
| POST /webhooks/stripe                                                                                              | Stripe                        | Verify signature; process each event id once (`stripe_events`); disputes created/closed → disputeLost/won. Hosted endpoint: the separate `stripe-webhook` function, `/functions/v1/stripe-webhook` (no JWT) |
