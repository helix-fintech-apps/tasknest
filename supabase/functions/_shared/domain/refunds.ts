...
export function planRefund(
  ...
  if (
    ctx.completedAt &&
    req.requestedAt > addDays(ctx.completedAt, policy.refunds.windowDays) &&
    req.actor !== "admin"
  ) {
  ...
  if (requiresApproval && !req.approvedBy)
    throw new Error("agent refund above limit needs approval");
  ...
  return {
    perTender,
    taskerClawbackCents: taskerClawback,
    platformCostCents: amount - taskerClawback,
    requiresApproval,
  };
}
...
export function disputeLost(
  disputedCents: Cents,
  cardPaid: Cents,
  cardAlreadyRefunded: Cents,
  ctx: { subtotal: Cents; total: Cents; taskerCommission: Cents },
  disputeFeeCents: Cents,
): DisputeOutcome {
  ...
  return {
    cardReversedCents: reversed,
    disputeFeeCents,
    recoverFromTaskerCents: recover,
    platformLossCents: reversed - recover + disputeFeeCents,
  };
}
