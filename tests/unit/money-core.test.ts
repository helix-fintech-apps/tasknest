import {
  DEFAULT_POLICY as P, quote, allocateTenders, splitRefund, refundAfterRetention, partsTotal,
  clientCancellation, taskerCancellation, shouldSuspend, localToUtc, pointsEarned, clawbackPoints,
  consumeFifo, newEarnLot, availablePoints, pendingPoints, planRefund, disputeLost, validateTip,
  planPayout, txn, dr, cr, canTransition, needsReauth, promoDiscount, divRoundHalfUp,
...
describe("split tender", () => {
  it("uses promo -> points -> wallet -> card", () => {
    const a = allocateTenders(10350, { promoCents: 1000, walletCents: 500, pointsBalance: 2000, pointsRequested: 1500 }, P);
  ...
  it("never makes the card negative and caps points at the remaining total", () => {
    const a = allocateTenders(800, { promoCents: 0, walletCents: 0, pointsBalance: 5000, pointsRequested: 5000 }, P);
    expect(a.parts.card).toBe(0);
    expect(a.pointsUsed).toBe(800);
  });
  it("rejects below-minimum redemptions", () => {
    expect(() => allocateTenders(5000, { promoCents: 0, walletCents: 0, pointsBalance: 1000, pointsRequested: 100 }, P)).toThrow();
  });
  it("splits partial refunds card first and never over-refunds a tender", () => {
    const paid = { card: 6000, points: 4000, wallet: 0, promo: 0 };
    const first = splitRefund(7000, paid, { card: 0, points: 0, wallet: 0, promo: 0 }, P.refundOrder);
...
describe("cancellation curve", () => {
  ...
  const b = { rateCents: 4500, subtotal: 9000, serviceFee: 1350, tax: 0, total: 10350, cutoffAnchorAt: start };
  ...
  it("converts local task time across DST correctly", () => {
    ...
    expect(localToUtc("2026-11-02T09:00", "America/Los_Angeles").toISOString()).toBe("2026-11-02T17:00:00.000Z");
    expect(localToUtc("2026-10-30T09:00", "America/Los_Angeles").toISOString()).toBe("2026-10-30T16:00:00.000Z");
  });
});
...
describe("points", () => {
  ...
  it("keeps earned points pending for 7 days and spends soonest-expiring first", () => {
    ...
    const lots = [newEarnLot("a", 600, done, P), newEarnLot("b", 400, new Date("2025-06-01T00:00:00Z"), P)];
    ...
    expect(consumeFifo(lots, 500, now)).toEqual([{ lotId: "b", points: 400 }, { lotId: "a", points: 100 }]);
  });
});
...
describe("refunds and disputes", () => {
  const ctx = {
    ...
    subtotal: 9000, total: 10350, taskerCommissionOnSubtotal: 1350, taskerPaidOut: false, disputeOpen: false,
  };
  const at = new Date("2026-11-12T00:00:00Z");
  it("partial refund splits card first and shares cost with the tasker", () => {
    const r = planRefund({ kind: "partial", amountCents: 2000, actor: "client", reason: "left early", requestedAt: at }, ctx, P);
  ...
  it("goodwill is absorbed by the platform", () => {
    const r = planRefund({ kind: "goodwill", amountCents: 1000, actor: "support_agent", reason: "late", requestedAt: at }, ctx, P);
    expect(r.taskerClawbackCents).toBe(0);
  });
  it("agent refunds above the limit need approval", () => {
    expect(() => planRefund({ kind: "partial", amountCents: 10001, actor: "support_agent", reason: "x", requestedAt: at }, ctx, P)).toThrow(/approval/);
  });
  it("blocks refunds while a dispute is open", () => {
    expect(() => planRefund({ kind: "full", amountCents: 1, actor: "admin", reason: "x", requestedAt: at }, { ...ctx, disputeOpen: true }, P)).toThrow();
  });
  it("lost dispute only reverses the card portion and recovers the tasker share", () => {
    const d = disputeLost(10350, 7350, 0, { subtotal: 9000, total: 10350, taskerCommission: 1350 }, 1500);
...
describe("tips, payouts, promo, ledger, state", () => {
  ...
  it("tips are card-only, capped at 25% and 100% to the tasker", () => {
    expect(validateTip(2250, "card", 9000, done, new Date("2026-11-11T00:00:00Z"), P)).toEqual({ taskerGets: 2250, platformFee: 0 });
    expect(() => validateTip(2251, "card", 9000, done, new Date("2026-11-11T00:00:00Z"), P)).toThrow();
    expect(() => validateTip(500, "points", 9000, done, new Date("2026-11-11T00:00:00Z"), P)).toThrow();
  });
  it("pays out only after the hold and never while suspended or negative", () => {
    const items = [{ bookingId: "b1", netCents: 7650, completedAt: done, cardSettled: true, paidOut: false }];
    expect(planPayout(items, 0, "active", new Date("2026-11-12T00:00:00Z"), P).amountCents).toBe(0);
    expect(planPayout(items, 0, "active", new Date("2026-11-14T00:00:00Z"), P).amountCents).toBe(7650);
    expect(planPayout(items, 0, "suspended", new Date("2026-11-14T00:00:00Z"), P).blockedReason).toMatch(/suspended/);
    expect(planPayout(items, -8000, "active", new Date("2026-11-14T00:00:00Z"), P).amountCents).toBe(0);
  });
  it("promo codes cap at the subtotal and can't be reused", () => {
    expect(promoDiscount({ code: "FIRST20", kind: "percent", value: 2000, firstTaskOnly: true }, 9000, true, false, done)).toBe(1800);
    expect(() => promoDiscount({ code: "FIRST20", kind: "percent", value: 2000, firstTaskOnly: true }, 9000, true, true, done)).toThrow();
  });
  it("rejects unbalanced ledger transactions", () => {
    expect(() => txn("bad", [dr("card_clearing", 100), cr("client_funds_held", 99)])).toThrow();
    expect(txn("ok", [dr("card_clearing", 100), cr("client_funds_held", 100)]).lines).toHaveLength(2);
  });
