...
function booking(id: string, status: string, startMs: number, extra: Row = {}): Row {
  ...
  return {
    id, client_id: IDS.ava, tasker_id: IDS.tara, policy_version: 1, status, description: "Mount TV", location_tz: "America/Los_Angeles",
    start_at: iso, original_start_at: iso, est_minutes: 120, rate_cents: 4500, subtotal_cents: 9000, service_fee_cents: 1350,
    tax_cents: 0, total_cents: 10350, extra_cents: 0, points_reserved: 0, points_earned: 0, promo_code: null,
    created_at: new Date(startMs - 100 * H).toISOString(), accepted_at: null, completed_at: null, canceled_at: null, ...extra,
  };
}

export function makeDb(now = Date.now()): Record<string, Row[]> {
  const profiles: Row[] = [
    { id: IDS.ava, role: "client", full_name: "Ava Client", home_tz: "America/Los_Angeles", email: "ava@tasknest.test" },
    { id: IDS.ben, role: "client", full_name: "Ben Client", home_tz: "America/New_York", email: "ben@tasknest.test" },
    { id: IDS.tara, role: "tasker", full_name: "Tara Tasker", home_tz: "America/Los_Angeles", email: "tara@tasknest.test" },
    { id: IDS.leo, role: "tasker", full_name: "Leo Tasker", home_tz: "America/Los_Angeles", email: "leo@tasknest.test" },
    { id: IDS.pia, role: "tasker", full_name: "Pia Tasker", home_tz: "America/Los_Angeles", email: "pia@tasknest.test" },
    { id: IDS.admin, role: "admin", full_name: "Admin", home_tz: "America/Los_Angeles", email: "admin@tasknest.test" },
    { id: IDS.agent, role: "support_agent", full_name: "Agent", home_tz: "America/Los_Angeles", email: "agent@tasknest.test" },
  ];
  ...
  const bookings = [
    ...
    booking(IDS.bDone, "completed", now - 3 * 24 * H, { completed_at: completedAt, points_earned: 103 }),
    booking(IDS.bReq, "requested", now + 5 * 24 * H, { client_id: IDS.ben, description: "Fix shelf" }),
    booking(IDS.bProg, "in_progress", now - 1 * H, { client_id: IDS.ben, description: "Assemble desk" }),
  ];
  const tenders: Row[] = [
  ...
  const lot = (id: string, pts: number, availOffset: number, expOffset: number, kind = "earn"): Row => ({
    id, user_id: IDS.ava, kind, booking_id: null, points_initial: pts, points_remaining: pts,
    available_at: new Date(now + availOffset).toISOString(), expires_at: new Date(now + expOffset).toISOString(), created_at: new Date(now - 30 * 24 * H).toISOString(),
  });
  return {
    profiles,
    taskers: [
      { id: IDS.tara, headline: "Mounting, repairs, furniture assembly", category: "Handyman", hourly_rate_cents: 4500, status: "active", kyc_verified_at: new Date(now).toISOString(), suspended_at: null },
      { id: IDS.leo, headline: "Deep cleaning and move-outs", category: "Cleaning", hourly_rate_cents: 3800, status: "active", kyc_verified_at: new Date(now).toISOString(), suspended_at: null },
      { id: IDS.pia, headline: "Moving help", category: "Moving", hourly_rate_cents: 6000, status: "pending", kyc_verified_at: null, suspended_at: null },
    ],
    money_policies: [{ version: 1, policy: DEFAULT_POLICY, effective_from: new Date(now - 90 * 24 * H).toISOString() }],
    bookings,
    booking_tenders: tenders,
    refunds: [], tips: [], reviews: [],
    points_lots: [
    ...
    points_movements: [
      { id: 1, user_id: IDS.ava, booking_id: null, lot_id: null, kind: "earn", points: 1500, created_at: new Date(now - 60 * 24 * H).toISOString() },
      { id: 2, user_id: IDS.ava, booking_id: IDS.bDone, lot_id: null, kind: "earn", points: 103, created_at: completedAt },
    ],
    promo_codes: [
      { code: "FIRST20", kind: "percent", value: 2000, first_task_only: true, max_discount_cents: 3000, expires_at: null },
      { code: "WELCOME10", kind: "fixed", value: 1000, first_task_only: false, max_discount_cents: null, expires_at: null },
    ],
    promo_redemptions: [],
    payouts: [], tasker_strikes: [], disputes: [], ledger_txns: [], ledger_lines: [],
...
function session(user: Row) {
  ...
  return {
    access_token: access, token_type: "bearer", expires_in: 3600, expires_at: exp, refresh_token: `refresh-${user.id}`,
  ...
  for (const [key, raw] of params.entries()) {
    ...
    out = out.filter((r) => {
      ...
      switch (op) {
        ...
        case "in": return val.replace(/^\(|\)$/g, "").split(",").map((x) => x.replace(/^"|"$/g, "")).includes(s);
...
export async function mockBackend(page: Page, db = makeDb()): Promise<MockBackend> {
  ...
  const cors = (route: Route) => route.fulfill({
    status: 204,
  ...
  await page.route("**/auth/v1/**", async (route: Route, req: Request) => {
    ...
    if (url.pathname.endsWith("/token")) {
      ...
      if (!user || body.password !== PASSWORD) return json(route, 400, { error: "invalid_grant", error_description: "Invalid login credentials", code: "invalid_credentials", msg: "Invalid login credentials" });
      currentUser = user;
      return json(route, 200, session(user));
    }
    if (url.pathname.endsWith("/user")) return currentUser ? json(route, 200, session(currentUser).user) : json(route, 401, { msg: "no user" });
    if (url.pathname.endsWith("/logout")) { currentUser = null; return route.fulfill({ status: 204, headers: { "access-control-allow-origin": "*" } }); }
  ...
  await page.route("**/rest/v1/**", async (route: Route, req: Request) => {
    ...
    if (single) {
      ...
      if (rows.length === 0 && req.headers()["accept"]?.includes("nulls=stripped") === false) return json(route, 406, { code: "PGRST116", message: "no rows" });
      return json(route, rows.length ? 200 : 406, rows[0] ?? { code: "PGRST116", message: "JSON object requested, multiple (or no) rows returned", details: "The result contains 0 rows" });
    }
  ...
  await page.route("**/functions/v1/api/**", async (route: Route, req: Request) => {
    ...
    const call: ApiCall = { method: req.method(), path, body: req.postDataJSON(), headers: req.headers() };
    if (path !== "/quote") apiCalls.push(call);
    const h = handlers.find((x) => x.re.test(path));
    if (h) { const r = h.handler(call); return json(route, r.status ?? 200, r.body); }
    if (path === "/quote") return json(route, 503, { error: { code: "unavailable", message: "mock: no server quote" } });
    const m = /^\/bookings\/([^/]+)\/(\w[\w-]*)$/.exec(path);
    if (m) {
      const b = db.bookings.find((x) => x.id === m[1]);
      const next: Record<string, string> = { accept: "accepted", decline: "declined", start: "in_progress", complete: "completed", cancel: "canceled_client" };
      if (b && next[m[2]]) b.status = next[m[2]];
      return json(route, 200, { booking: b ?? { id: m[1], status: "unknown" } });
    }
    if (path === "/bookings") return json(route, 200, { booking: { id: "10000000-0000-4000-8000-00000000beef", status: "requested" } });
