...
export interface QuoteRequest { taskerId: string; minutes: number; promoCode?: string; pointsRequested?: number }
...
export interface BookingResponse { booking: { id: string; status: string; [k: string]: unknown }; [k: string]: unknown }
...
export interface CompleteRequest { extraMinutes?: number; expensesCents?: number }
export interface RefundRequestBody { kind: RefundKind; amountCents: number; reason: string; approvedBy?: string }
export interface GenericResponse { [k: string]: unknown }

const enc = encodeURIComponent;

export const api = {
  quote: (b: QuoteRequest) => call<QuoteResponse>("POST", "/quote", b, newIdempotencyKey()),
  createBooking: (b: CreateBookingRequest, key: string) => call<BookingResponse>("POST", "/bookings", b, key),
  accept: (id: string, key: string) => call<BookingResponse>("POST", `/bookings/${enc(id)}/accept`, {}, key),
  decline: (id: string, key: string) => call<BookingResponse>("POST", `/bookings/${enc(id)}/decline`, {}, key),
  reschedule: (id: string, localStart: string, key: string) =>
    call<BookingResponse>("POST", `/bookings/${enc(id)}/reschedule`, { localStart }, key),
  cancel: (id: string, reason: string, key: string) => call<CancelResponse>("POST", `/bookings/${enc(id)}/cancel`, { reason }, key),
  noShow: (id: string, who: "client" | "tasker", key: string) => call<GenericResponse>("POST", `/bookings/${enc(id)}/no-show`, { who }, key),
  start: (id: string, key: string) => call<BookingResponse>("POST", `/bookings/${enc(id)}/start`, {}, key),
  complete: (id: string, b: CompleteRequest, key: string) => call<BookingResponse>("POST", `/bookings/${enc(id)}/complete`, b, key),
  tip: (id: string, amountCents: number, key: string) => call<GenericResponse>("POST", `/bookings/${enc(id)}/tip`, { amountCents }, key),
  refund: (id: string, b: RefundRequestBody, key: string) => call<GenericResponse>("POST", `/bookings/${enc(id)}/refund`, b, key),
  ...
  setTaskerStatus: (taskerId: string, status: "active" | "suspended", reason: string, key: string) =>
    call<GenericResponse>("POST", `/admin/taskers/${enc(taskerId)}/status`, { status, reason }, key),
