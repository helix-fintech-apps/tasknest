...
export default function TaskerProfile() {
  ...
  const lots = useLoad(
    () => (profile ? pointsLots(profile.id) : Promise.resolve([])),
    [profile?.id],
  );
  const firstTask = useLoad(
    () => (profile ? clientHasBookings(profile.id).then((has) => !has) : Promise.resolve(true)),
    [profile?.id],
  );
  ...
  const pointsBalance = useMemo(
    () => Math.max(0, pointsSummary(lots.data ?? [], new Date()).availableNet),
    [lots.data],
  );
    ...
    const t = setTimeout(async () => {
      try {
        const [row, used] = await Promise.all([
          getPromo(code),
          profile ? promoUsed(profile.id, code) : Promise.resolve(false),
        ]);
  ...
  const preview = useMemo(() => {
    ...
    return previewQuote({
      rateCents: Number(t.hourly_rate_cents),
      minutes,
      policy,
      promo: promo && promo.code === promoCode ? promo.row : null,
      promoCodeEntered: promo && promo.code === promoCode ? promoCode : "",
      isFirstTask: firstTask.data ?? false,
      promoAlreadyUsed: promo?.used ?? false,
      pointsRequested,
      pointsBalance,
      walletCents: 0,
      now: new Date(),
  ...
  const book = useAction((key: string) =>
    api.createBooking(
      {
        taskerId: id,
        localStart,
  ...
  const mismatch =
    ...
    q &&
    alloc &&
    (serverTotal !== q.total || (serverCard !== undefined && serverCard !== alloc.parts.card));
  ...
  const canBook =
    ...
    !!alloc &&
    !preview?.promoError &&
    !preview?.allocationError &&
    description.trim().length > 0 &&
    !!localStart;
  const avgRating = reviews.data?.length
    ? reviews.data.reduce((a, r) => a + r.rating, 0) / reviews.data.length
    : null;
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
      <div className="space-y-6">
        <Card>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-slate-900" data-testid="tasker-name">
                {t.name}
              </h1>
              <div className="text-sm text-slate-500">
                {t.category} · <span data-testid="tasker-rate">{money(t.hourly_rate_cents)}/h</span>{" "}
        ...
        <Card>
          ...
          {!isClient && (
            <Banner tone="info">
              Only client accounts can book. You can still preview the price.
            </Banner>
          )}
          <div className="grid gap-x-4 sm:grid-cols-2">
            ...
            <Field
              ...
              hint={
                preview?.promoError ? (
                  <span className="text-red-600" data-testid="promo-error">
                    {preview.promoError}
                  </span>
                ) : preview?.discountCents ? (
                  <span className="text-emerald-700">Applied: −{money(preview.discountCents)}</span>
                ) : undefined
              }
            >
              <Input
          ...
          <Field label="Describe the task" id="desc">
            <Textarea
              ...
              placeholder='e.g. Mount a 55" TV on drywall'
            />
          </Field>
            ...
            <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked && !pointsInput && q)
                    setPointsInput(
                      String(
                        Math.min(
                          pointsBalance,
                          Math.floor(q.total / (policy?.points.centsPerPoint || 1)),
                        ),
                      ),
                    );
                }}
              ...
              <span className="font-normal text-slate-500">
                (<span data-testid="points-available">{pointsBalance.toLocaleString()}</span>{" "}
                available)
              </span>
            </label>
            {usePoints && (
              <div className="mt-2">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Minimum {policy?.points.minRedeemPoints} points · 1 point ={" "}
                  {money(policy?.points.centsPerPoint ?? 1)} · points are reserved now and redeemed
                  when the task completes.
                </p>
              </div>
            )}
            {preview?.allocationError && (
              <p className="mt-2 text-sm text-red-600" data-testid="points-error">
                {preview.allocationError}
              </p>
            )}
          </div>
          {book.error && (
            <div className="mt-4">
              <Banner tone="error" testId="booking-error">
                {book.error}
              </Banner>
            </div>
          )}
            ...
            onClick={async () => {
              ...
              if (r)
                nav("/bookings", {
                  state: {
                    flash:
                      "Booking requested. Your card is authorized, not charged, until the task is done.",
                  },
                });
            }}
          >
            {book.pending
              ? "Booking…"
              : q
                ? `Request booking · ${money(q.total)}`
                : "Request booking"}
          </Button>
          <p className="mt-2 text-xs text-slate-500">
            Your card is authorized now and captured when the task is completed. See{" "}
            <a className="text-blue-700 underline" href="/pricing">
              pricing & policies
            </a>{" "}
            for the cancellation curve.
          </p>
        </Card>
        ...
        {!!reviews.data?.length && (
          <Card>
            ...
            <ul className="space-y-3">
              {reviews.data.map((r) => (
                <li key={r.booking_id} className="text-sm">
      ...
      <aside>
        <Card className="sticky top-4" data-testid="quote-panel">
          <H2>Price details</H2>
          {!q ? (
            <p className="text-sm text-slate-500">{preview?.error ?? "—"}</p>
          ) : (
            <>
              <MoneyRow
                label={`${money(t.hourly_rate_cents)}/h × ${fmtMinutes(minutes)}`}
                cents={q.subtotal}
                testId="quote-subtotal"
              />
              <MoneyRow
                label={`Service fee (${policy!.clientServiceFeeBps / 100}%)`}
                cents={q.serviceFee}
                testId="quote-service-fee"
              />
              ...
              {alloc ? (
                <div data-testid="quote-tenders">
                  <MoneyRow
                    label="Promo credit"
                    cents={alloc.parts.promo}
                    testId="quote-promo"
                    muted={!alloc.parts.promo}
                  />
                  <MoneyRow
                    label={`Points${alloc.pointsUsed ? ` (${alloc.pointsUsed.toLocaleString()} pts)` : ""}`}
                    cents={alloc.parts.points}
                    testId="quote-points"
                    muted={!alloc.parts.points}
                  />
                  <MoneyRow
                    label="Wallet"
                    cents={alloc.parts.wallet}
                    testId="quote-wallet"
                    muted={!alloc.parts.wallet}
                  />
              ...
              <div className="mt-3 text-xs" data-testid="quote-server-status">
                {server.state === "loading" && (
                  <span className="text-slate-400">Confirming with server…</span>
                )}
                {server.state === "ok" && !mismatch && (
                  <span className="text-emerald-700">✓ Server quote matches</span>
                )}
                {mismatch && (
                  <span className="text-red-600" data-testid="quote-mismatch">
                    Server quote differs: total {money(serverTotal)} · card {money(serverCard)}. The
                    server amount is what you'll be charged.
                  </span>
                )}
                {server.state === "error" && (
                  <span className="text-slate-400">
                    Server quote unavailable ({server.error}). Preview only.
                  </span>
                )}
              </div>
              <p className="mt-3 text-xs text-slate-400">
                Policy v{row?.version ?? policy?.version}
                {source === "builtin" && " (built-in copy — policy table unreachable)"}.
              </p>
            </>
          )}
        </Card>
